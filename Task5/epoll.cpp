...
void open_server(int server) {
    ...
    int result = ::bind(server, reinterpret_cast<::sockaddr*>(&addr), sizeof(addr));
...
int main() {
    ...
    while(true) {
        int events_count = ::epoll_wait(*epoll, event_queue.data(), event_queue.size(), -1);
        ...
        for(auto it = start; it != end; ++it) {
            if(it->data.fd == *server) {
                ...
                std::cout << "From client " << it->data.fd << ": " << buf.substr(0, read) << std::endl;
            }
        }
    }
